...
export default async function WomanFashion() {
  const products: IProduct[] = await apiServices.getProducts();
  const womenFashionProducts = products.filter(
  ...
  return (
    <div>
          ...
          <div className="flex gap-3 items-center">
            ...
            <div>
              ...
              <p className="text-white text-sm">
                Shop from women's fashion category
              </p>
            </div>
          </div>
        </div>
      ...
      <p className="pt-5 mb-5 px-30 text-gray-800 text-lg">
        Showing {womenFashionProducts.length} products
      </p>
      <div className="px-30 pb-10 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        {womenFashionProducts.length === 0 ? (
          ...
          womenFashionProducts.map((product) => (
